...
var Repeater = function (_props, _hideComponents = false) {
  ...
  this.containerKeyDown = function (e) {
    if (typeof _keydown == "function") _keydown.apply(this, arguments);
        ...
        if (len > 0) {
          for (let i = 0; i < len; i++) {
            ...
            if (data != null) {
              ...
              _comprenders.splicea(
                _comprenders.length,
                0,
                rowComponentsPromises
              );
            }
          }
        } else _creationFinished = true;
      } else _creationFinished = true;
  ...
  let _rowAdded = function (index, data) {
    ...
    _self.trigger(era, [
      _self,
      new RepeaterEventArgs(_rowItems[index], data, index),
    ]);
  ...
  if (!this.hasOwnProperty("dataProviderChanged")) {
    this.dataProviderChanged = function (toAdd, toRemove, toRefresh) {
      ...
      for (let i = 0; i < toRemove.a1_indices.length; i++) {
        ...
        if (
          toRefresh.indexOf(toRemove.a1_indices[i]) == -1 &&
          toAdd.a1_indices.indexOf(toRemove.a1_indices[i]) == -1
        )
          if (toRemove.a1_indices[i] < _rows.length)
            this.removeRow(
              toRemove.a1_indices[i],
              false,
              true,
              (dpRemove = false)
            );
      ...
      if (toAdd.a1_indices.length > 0) {
        ...
        for (let i = 0; i < toAdd.a1_indices.length; i++) {
          if (
            toRefresh.indexOf(toAdd.a1_indices[i]) == -1 &&
            toRemove.a1_indices.indexOf(toAdd.a1_indices[i]) == -1
          ) {
            if (!transferNodes) {
            ...
            let rowComponentsPromises = this.addRow(
              this.dataProvider[ind],
              ind
            );
            Promise.all(rowComponentsPromises).then(function () {
    ...
    set: function dataProvider(v) {
      if (!_dataProvider || _dataProvider != v) {
        ...
        if (v == null || v.length == 0) {
          ...
          _oldDataProvider &&
          _oldDataProvider.length > 0 &&
          _oldDataProvider.length == this.rowItems.length
        ) {
          let delta =
            (v.length ? v.length : 0) -
            (_oldDataProvider.length ? _oldDataProvider.length : 0);
          for (
            let ri = 0;
            ri < Math.min(v.length, _oldDataProvider.length);
            ri++
          ) {
  ...
  let _dpLengthChanged = function (e) {
    ...
    if (_creationFinished) {
      ...
      if (
        newValue < oldValue &&
        toRemove.result.length != oldValue - newValue
      ) {
    ...
    let dataProviderLengthChangedEvent = jQuery.Event(
      "dataProviderLengthChanged"
    );
  ...
  let _dpMemberChanged = function (e) {
    ...
    if (
      _creationFinished &&
      ["length", "guid", "filterData"].indexOf(e.property) == -1
    ) {
      ...
      get: function value() {
        ...
        for (var i = 0; i < _components.length; i++) {
          ...
          for (let j = 0; j < this[_components[i].props.id].length; j++) {
            value[_components[i].props.id].push(
              this[_components[i].props.id][j].value
            );
          }
        }
        return value;
  ...
  this.addRow = function (
    ...
    this.trigger(beforeRowAddEvent, [
      _self,
      new RepeaterEventArgs(_rowItems, data, index),
    ]);

    if (
      !isPreventable ||
      (isPreventable && !beforeRowAddEvent.isDefaultPrevented())
    ) {
      let len = _components.length;
      for (let cIndex = 0; cIndex < len; cIndex++) {
        let component = {};
        if (!_components[cIndex].props.id) {
          _components[cIndex].props.id =
            functionName(_components[cIndex].ctor) +
            "_" +
            (Component[_components[cIndex].ctor]
              ? Component[_components[cIndex].ctor].instanceInc
              : 0);
        }
        shallowCopy(_components[cIndex], component, ["props"]);
        component.props = {};
        shallowCopy(_components[cIndex].props, component.props, [
          "id",
          "bindingDefaultContext",
        ]);
        component.props.id =
          _components[cIndex].props.id + "_" + index + "_" + cIndex;
        ...
        el.on("focus", function (e, repeaterEventArgs) {
          ...
          _focusedComponent = Object.keys(repeaterEventArgs.currentRow).indexOf(
            this.id
          );
          console.log(
            "focused repeated component",
            _focusedRow,
            _focusedComponent
          );
        });
        ...
        el.on("change", function (e, rargs) {
          ...
          if (
            ...
            isString(component.props.value) &&
            component.props.value[0] == "{" &&
            component.props.value[component.props.value.length - 1] == "}"
          ) {
            ...
            if (bindingExp == "currentItem") {
              ...
              setChainValue(
                _dataProvider[rargs.currentIndex],
                bindingExp,
                this.value
              );
          ...
          _self.trigger("rowEdit", [
            _self,
            new RepeaterEventArgs(rowItems, data, index),
          ]);
        });
        //rowItems = {};
          ...
          let cp = el.render().then(function (cmpInstance) {
            if (!_rendering.wrap) {
              ...
              renderedRow
                ...
                .css(
                  _rendering.direction == "horizontal"
                    ? {
  ...
  if (!this.hasOwnProperty("removeRow")) {
    this.removeRow = function (
      ...
      if (
        !isPreventable ||
        (isPreventable && !beforeRowDeleteEvent.isDefaultPrevented())
      ) {
        //remove dp row
        var removedItem;
        if (dpRemove) {
        ...
        for (var cI = 0; cI < _components.length; cI++) {
          ...
          if (cI == 0 && _rendering.wrap) {
            this[component.props.id][index].$el
              .closest(".repeated-block")
              .remove();
            this[component.props.id][index].$el
              .closest(".repeated-block-hr")
              .remove();
          }
          if (!_rendering.wrap) {
          ...
          for (var i = 0; i < this[component.props.id].length; i++) {
            ...
            if (i > index) item.repeaterIndex -= 1;
          }

          rowItems[component.props.id] = [this[component.props.id][index]];
          this[component.props.id].splice(index, 1);
        }
        this.trigger("rowDelete", [
          this,
          new RepeaterEventArgs(rowItems, this.dataProvider[index], index),
        ]);
  ...
  this.render = function () {
    ...
    else this.dataProvider = new ArrayEx([]);
    return _rPromise;
  };
