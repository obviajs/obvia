...
oxana.behaviorimplementations["SPLIT_HOR"] = function(e) {
    ...
    var newRow = {
        ...
        var newColInstance = newRowInstance.children[newRowInstance.components[0].props.id];
        var newWorkArea = newColInstance.children[newColInstance.components[0].props.id];
        ...
        {
            var newRow2 = Object.assign({}, newRow);
            var newRowInstance2 = activeContainer.addComponent(newRow2);
            var newColInstance2 = newRowInstance2.children[newRowInstance2.components[0].props.id];
            var newWorkArea2 = newColInstance2.children[newColInstance2.components[0].props.id];
...
oxana.behaviorimplementations["SPLIT_VERT"] = function(e) {
    ...
    var children_len = activeContainer.parent.parent.components.length;
    if(children_len<12)
    {
        ...
        var newColInstance = activeContainer.parent.parent.addComponent(newCell);
        var newWorkArea = newColInstance.children[newColInstance.components[0].props.id];
        oxana.behaviors[newWorkArea.id] = {};
        oxana.behaviors[newWorkArea.id]["click"] = "BECOME_ACTIVE";    
        ++children_len;
        ...
        var i = 0;    
        for(var childID in activeContainer.parent.parent.children){
            ++i;
            if(i==children_len-1)
                activeContainer.parent.parent.children[childID].spacing.colSpan = colSpan+delta;
            else
                activeContainer.parent.parent.children[childID].spacing.colSpan = colSpan;
        } 

    }else
    {
        alert("You may have up to 12 columns for each cell.");
    }
       
};

var activeContainer;
oxana.behaviorimplementations["BECOME_ACTIVE"] = function(e) {
    ...
    if(activeContainer && activeContainer!=this){
        activeContainer.toggle = true;
        activeContainer.classes = ["active"];
    }
    this.toggle = true;
    this.classes = ["active"];
