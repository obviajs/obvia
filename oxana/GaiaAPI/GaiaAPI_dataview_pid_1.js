var GaiaAPI_dataview_pid_1 = function (_props) {
  ...
  let _server = _props.server,
    _apiClient = _props.apiClient ? _props.apiClient : new ApiClient(),
    _self = this;
  ...
  this.dataview_pid_1 = function (apiClient) {
    ...
    this.post = function (dvInput) {
      ...
      let responses = {
        200: { responseType: "JSON", type: "arrayForm" },
        404: { responseType: "JSON", type: "responseStatus" },
        500: { responseType: "JSON", type: "responseStatus" },
      };
      return new Promise((resolve, reject) => {
        this.apiCall(objQuery, objBody, objPath, requestContentType, "post")
          .then(function (resp) {
            if (responses[resp.status]) {
              ...
              switch (responseType) {
                case "json":
                  ret = isString(resp.response)
                    ? JSON.parse(resp.response)
                    : resp.response;
                  ret = new GaiaAPI_dataview_pid_1[responses[resp.status].type](
                    ret
                  );
                  break;
              }
              ...
              resolve(ret);
            } //unspecified http response code returned
            else reject();
          })
