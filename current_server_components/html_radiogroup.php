 ...
 */
class html_radiogroup extends html_element {
    ...
    public function render() {
        ...
        return 
			"<div class='form-group col-lg-{$this->col_span} rowspan{$this->row_span} resizable' id=\"{$this->field_name}_container\" >
				...
				{$this->createRadioGroupOptions()} ".
                $this->html_tooltip() . "
			</div></div>";
    }
	
	public function createRadioGroupOptions()
	{
		$html = "<div class='panel panel-default'>
					<div class='panel-body'>";
		foreach($this->optionsData as $optData)
		{
			if($this->selectedValue == $optData->id)
			{
				$html .= "<div class='radio'>
							<label><input type='radio' name='{$this->field_name}' id='{$optData->val}' value='{$optData->id}' checked>{$optData->val}</label>
						</div>";
			}
			else
			{
				$html .= "<div class='radio'>
								<label><input type='radio' name='{$this->field_name}' id='{$optData->val}' value='{$optData->id}' >{$optData->val}</label>
							</div>";
			}
		}
		$html .= "</div>
