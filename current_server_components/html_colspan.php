 ...
 */
class html_colspan extends html_element {
    /**
     * Funksioni render()
     * 
    ...
    public function render() {
        if(isNullOrEmpty($this->col_span) || $this->col_span == 0)
			$this->col_span = 2;
		return 
            "<div class='col-lg-{$this->col_span} rowspan{$this->row_span} resizable' style='background-color: #ccc; margin-bottom: 5px;'><label></label></div>";
    ...
    public function render_for_view() {
        if(isNullOrEmpty($this->col_span) || $this->col_span == 0)
			$this->col_span = 2;
		return 
            "<div class='col-lg-{$this->col_span} rowspan{$this->row_span}'></div>";
			// <label for='{$this->field_name}'>{$this->label}</label>
    }
    /**
