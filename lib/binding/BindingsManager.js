            ...
            if (_site != null && site_chain != undefined && Array.isArray(site_chain) && site_chain.length > 0) {
                ...
                for (let i = 0; i < toBind.length; i++) {
                    ...
                    if (context[host]) {
                        ...
                        _chainUpdateFns[chain].push({ "watcher": _watchers[_watchersCount], "handler": _bindingFunctions[_totalBindings] });
...
BindingsManager.getIdentifier = function (bindingExpression) {
    let r = null;
