    ...
    this.getValue = function (context, bindingExpression, site_chain) {
        ...
        if (toBind) {
            ...
            if (_site != null && site_chain != undefined && Array.isArray(site_chain) && site_chain.length > 0) {
                ...
                let timesExecuted = 0;
                _bindingFunctions[_totalBindings] = eval(`var f = function (e) {
                    try{
                        ${varsIn_defaultContextProperty}
                        ObjectUtils.setChainValue(_site, site_chain, (${bindingExpression}));
                    }catch(error){
...
BindingsManager.parse = function (tokens) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        if ((tokens[i].type === "IDENTIFIER") || ((tokens[i - 1] && tokens[i - 1].type === "LEFT_BRACKET") && ((tokens[i].type === "STRING" && tokens[i].value.trim() != "") || (tokens[i - 2] && tokens[i - 2].type != "RIGHT_PAREN" && tokens[i].type === "INTEGER")))) {
