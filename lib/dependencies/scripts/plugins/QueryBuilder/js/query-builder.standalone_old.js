...
QueryBuilder.prototype.bindEvents = function() {
    ...
    this.$el.on('change.queryBuilder', Selectors.rule_operator, function() {
        ...
        if(Model($rule).operator.type == 'equal' || Model($rule).operator.type == 'not_equal' || Model($rule).operator.type == 'less' || Model($rule).operator.type == 'less_or_equal' || Model($rule).operator.type == 'greater' || Model($rule).operator.type == 'greater_or_equal' || Model($rule).operator.type == 'between' || Model($rule).operator.type == 'not_between'){
            $('#_field_compare_' + Model($rule).id + 0).css('display', 'block');
            $('#field_compare_' + Model($rule).id + 0).prop('checked', false);
            $('#switch_input_' +  Model($rule).id + 0).css('display', 'none');
            $('#input_' +  Model($rule).id + 0).css('display', 'block');
        }
        else{
            $('#field_compare_' + Model($rule).id + 0).prop('checked', false);
            $('#_field_compare_' + Model($rule).id + 0).css('display', 'none');
            $('#switch_input_' +  Model($rule).id + 0).css('display', 'none');
            $('#input_' +  Model($rule).id + 0).css('display', 'block');
        }
    });
...
QueryBuilder.prototype.createRuleFilters = function(rule) {
     ...
     */
    rule.$el.find(Selectors.filter_container).children().select2({ dropdownAutoWidth: true });
...
QueryBuilder.prototype.updateRuleFilter = function(rule) {
    ...
    rule.$el.find(Selectors.rule_filter).select2('val', rule.filter ? rule.filter.id : '-1');
...
QueryBuilder.prototype.updateRuleOperator = function(rule, previousOperator) {
    ...
    if (rule.operator) {
        rule.$el.find(Selectors.rule_operator).select2('val', rule.operator.type);
    }
...
QueryBuilder.prototype.getRules = function() {
    ...
    var out = (function parse(group) {
        ...
        group.each(function(model) {
            ...
            for (var i=0; i<model.operator.nb_inputs; i++) {
                if(model.operator.nb_inputs>1){
                    if($('#field_compare_' + model.id + i).is(':checked')){
                        rule.value[i] = $('#switch_input_' + model.id + i).val();
                        rule.self += (i + 1);
                    }
                }
                else{
                    if($('#field_compare_' + model.id + 0).is(':checked')){
                        rule.value = $('#switch_input_' + model.id + 0).val();
                        rule.self = 0;
                    }
                }
            }
...
{{ var optgroup = null; }} \
<select class="form-control form-control-custom" name="{{= it.rule.id }}_filter"> \
...
<select class="form-control form-control-custom" name="{{= it.rule.id }}_operator"> \
...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
    ...
    for (var i = 0; i < this.settings.filters.length; i++){
       ...
       /* if( rule.__.filter.input == this.settings.filters[i].input && rule.__.filter.type != "date" && this.settings.filters[i].type != "date" && rule.__.filter.id != this.settings.filters[i].id){
            h += '<option value="' + this.settings.filters[i].id + '">' + this.settings.filters[i].label + '</option>';
            exists++;
        }
        && rule.__.filter.type == "date"
        */
    }
    h += '</select></span>';
                ...
                if (rule.rules && rule.rules.length>0) {
                    ...
                    if (ope.nb_inputs !== 0) {
                        ...
                        rule.value.forEach(function(v, i) {
                            ...
                            if ((rule.type=='integer' || rule.type=='double' || rule.type=='boolean') && $('#field_compare_' + rule.input_id + i).is(':checked') == false) {
