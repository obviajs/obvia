        ...
        return function (props, debug = false) {
            ...
            if (!component.hasOwnProperty('scrollTo')) {
                component.scrollTo = function () {
                    $("body").animate({
                        scrollTop: component.$el.parent().offset().top - 100
                    }, 500);
                    return this;
                }
            }
            ...
            rivets.bind(component.$el, model);
             ...
             //component jquery wrapper for event dispatching
            ...
            ready('#' + component.id, function (element) {
                ...
                //prevent defaults
                for (var i = 0; i < innerElementEvents.length; i++) {
                    (function (i) {
                        component.$el.on(innerElementEvents[i], function (event) {
                            event.preventDefault();
                            event.stopImmediatePropagation();
                            event.stopPropagation();
                            console.log("handling event from KxGenerator");   
                            
                            //execute inner handlers if theres any registered
                            if (typeof component.registerEvents == 'function') 
                            {
                                var handlers = component.registerEvents();
                                if(typeof handlers[innerElementEvents[i]] == 'function')
                                {
                                    handlers[innerElementEvents[i]].call(component.$component, event);
                                }
                            }
                            
                            if (component.parentType == 'repeater')
                                component.$component.trigger(
                                    innerElementEvents[i],
                                    [
                                        component,
                                        component.parent,
                                        new RepeaterEventArgs(
                                            component.parent.rowItems[component.repeaterIndex],
                                            component.parent.dataProvider.items[component.repeaterIndex],
                                            component.repeaterIndex
                                        )
                                    ]
                                );
                            else
                                component.$component.trigger(innerElementEvents[i], component);
                        });
                    })(i);
                }
                //remove on from here 

                //component.$el.trigger('afterAttach');
                component.$component.trigger('afterAttach');

            });
            /*
             //register inner handlers
             if (typeof component.registerEvents == 'function') {
                var handlers = component.registerEvents();
                for (var handler in handlers) {
                    component.$component.on(handler, handlers[handler]);
                }
            }*/
            ...
            component.on = function (eventType, fnc) {
                if (typeof fnc !== 'function') {
                    ...
                    component.$component.on(eventType, fnc);
            }
