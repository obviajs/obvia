           ...
           // {
                if (Object.prototype.hasOwnProperty.call(obj, prop) && !excludeProperties.includes(prop)) {
                    if(!(isGetter (obj, prop) || isSetter (obj, prop))){
                        ...
                        } if(deep && (Object.prototype.toString.call(obj[prop]) === '[object Array]') && !staticProperties.includes(prop)){
...
function getChainValue(host, chain){
    var pieces = parseObjectPathExpression(chain);
    ...
    for(var i=0;i<pieces.length;i++){
        if(curObj[pieces[i]]!=undefined){
            ...
            console.log(pieces[i]+" not found on "+chain+" ObjectPath")
        }
    }
    return curObj;
}
