function buildDefaultObject(obj) {
    var dI = {};
    for (var key in obj) {
        if (isObject(obj[key])){
            var o = {};
            for(let p in obj[key]){
                if(obj[key][p] !=null && typeof obj[key][p] == "object")
                    o[p] = this.buildDefaultItem([obj[key][p]]);	
                else	
                    o[p] = obj[key][p];
            }
            dI[key] = o;
            ...
            for(let i=0;i<obj[key].length;i++){
                if(obj[key][i] !=null && typeof obj[key][i] == "object")
                    a.push(this.buildDefaultItem([obj[key][i]])); 	
                else
                    a.push(obj[key][i]);
            } 
            dI[key] = a; 
                    ...
                    if(!(isGetter (obj, prop) || isSetter (obj, prop))){
                        ...
                        if (deep && (isObject(obj[prop]) && (obj[prop]["forEach"]==null) && !staticProperties.includes(prop))) {
                            if(!(keepDefined.includes(prop) && (obj[prop]==null || obj[prop]==undefined))){
                                ...
                                if (index === -1) {
                                    visited.push(value);
                                    extended[prop] = new Array((obj[prop]).length);
                                    set.push({up: extended, value: extended[prop]});
                        ...
                        } else if(isObject(obj[prop]) && (obj[prop]["forEach"]==null) && !((value.contructor && avoid.indexOf(value.contructor.name)>-1) || (value && (value.jquery!=null || value.jQuery!=null))) && !((extended[prop] && extended[prop].contructor && avoid.indexOf(extended[prop].contructor.name)>-1) || (extended[prop] && (extended[prop].jquery!=null || extended[prop].jQuery!=null)))){
                            var index = visited.indexOf(value);
                            if (index === -1) {
                                visited.push(value);
                                extended[prop] = extend(false, copyAccessors_flag, extended[prop], obj[prop]);
                                set.push({up: extended, value: extended[prop]});
                            }else{
                                extended[prop] = set[index].value;
                            }
...
function getChainValue(host, chain){
    ...
    var curObj = host;
    for(var i=0;i<chain.length && curObj!=null;i++){
...
function setChainValue(host, chain, value) {
    ...
    curObj[chain[chain.length-1]] = value;
}
...
function deepEqual(x, y, matchAllKeys = false) {
