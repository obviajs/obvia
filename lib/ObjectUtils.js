...
var ObjectUtils = {};
ObjectUtils.getMatchingMember = function (object, key, matchingValue, returnAllMatches = false) { 
    let chain = [], objects = [];
    for (var prop in object) { 
        if (ObjectUtils.getChainValue(object[prop], key) == matchingValue) { 
            chain.push([prop]);
...
*/
ObjectUtils.buildDefaultObject = function(obj, v) {
        ...
        if (ObjectUtils.isObject(obj[key])){
            ...
            for(let p in obj[key]){
                if(obj[key][p] !=null && typeof obj[key][p] == "object"){
                    ...
                    if(ind==-1 || ind >= len-1)
                        o[p] = ObjectUtils.buildDefaultObject(obj[key][p], v);	
                    else
                        o[p] = obj[key][p];
                }
                else	
            ...
            for(let i=0;i<obj[key].length;i++){
                if(obj[key][i] !=null && typeof obj[key][i] == "object"){
                    ...
                    if(ind==-1 || ind >= len-1)
                        a.push(ObjectUtils.buildDefaultObject(obj[key][i]), v); 	
                    else
                        a.push(obj[key][i]);
                }
                else
 ...
 */
ObjectUtils.shallowCopy = function (src, dest, excludeProperties = []) {
    for (let prop in src) {
...
ObjectUtils.inTheImageOf = function (src, dest) {
    let visited = [];
    for (let prop in dest) {
        if (!ObjectUtils.isObject(dest[prop])) {
            visited.push(prop);
            dest[prop] = src ? src[prop]:null;
        } else {
            visited.push(prop);
            ObjectUtils.inTheImageOf(src[prop], dest[prop]);
        }
    }
    if (ObjectUtils.isObject(dest)) {
        for (let prop in src) {
            if (visited.indexOf(prop) < 0) {
                if (!ObjectUtils.isObject(src[prop])) {
                    dest[prop] = src ? src[prop]:null;
                } else {
                    if (!dest[prop])
                        dest[prop] = {};
...
ObjectUtils.fromDefault = function (src, dest) {
    let visited = [];
    if (!ObjectUtils.isObject(dest)) {
        dest = {};
    }
    for (let prop in src) {
        if (visited.indexOf(prop) < 0) {
            if (!ObjectUtils.isObject(src[prop]) || src[prop].forEach) {
                if(dest[prop] == null)
                    dest[prop] = src[prop];
            } else {
                if (!dest[prop])
                    dest[prop] = {};
                ObjectUtils.fromDefault(src[prop], dest[prop]);
            }
        }
    }
    return dest;
...
*/
ObjectUtils.getMembersCollection = function(src, labelField, valueField){
    let c = [];
    for(let p in src){
        let o = {};
        o[labelField] = p;
 ...
 */ 
ObjectUtils.deepCopy = function (src, /* INTERNAL */ _visited, _copiesVisited) {
    ...
    if (Object.prototype.toString.call(src) == '[object Array]') {
        ...
        var i = ret.length;
        while (i--) {
            ret[i] = ObjectUtils.deepCopy(ret[i], _visited, _copiesVisited);
        }
        return ret;
    }
    ...
    for (var key in src) {
        ...
        if (src[key] && src[key].constructor && avoid.indexOf(src[key].constructor.name) == -1) {
            dest[key] = ObjectUtils.deepCopy(src[key], _visited, _copiesVisited);
        } else
            dest[key] = src[key];
           ...
           // {
                if (Object.prototype.hasOwnProperty.call(obj, prop) && !excludeProperties.includes(prop)) {
                    if(!(isGetter (obj, prop) || isSetter (obj, prop))){
                        ...
                        if (deep && (ObjectUtils.isObject(obj[prop]) && (obj[prop]["forEach"]==null) && !staticProperties.includes(prop) && !value.$el)) {
                            if(!(keepDefined.includes(prop) && (obj[prop]==null || obj[prop]==undefined))){
                                var index = visited.indexOf(value);
                                if (index === -1) {
                                    visited.push(value);
                                    extended[prop] = ObjectUtils.extend(true, copyAccessors_flag, obj[prop]);
                                ...
                                if (index === -1) {
                                    ...
                                    for(var j=0;j<(obj[prop]).length;j++){
                                        if((obj[prop][j] && obj[prop][j]["forEach"]) || ObjectUtils.isObject(obj[prop][j]))
                                            extended[prop][j] = ObjectUtils.extend( true, copyAccessors_flag, obj[prop][j]);
                                        else
                                ...
                                if(!extended.hasOwnProperty(prop) || staticProperties.includes(prop)){
                                   // extended[prop] = extend(false, copyAccessors_flag, obj[prop]);
                                    ...
                                    for(let p in obj[prop]){
                                        if(!extended[prop].hasOwnProperty(p) && ObjectUtils.isObject(obj[prop][p])){
                                            if(obj[prop][p]["forEach"]==null){
                                                extended[prop][p] = ObjectUtils.extend(false, copyAccessors_flag, obj[prop][p]);
                                            }else{
                                        ...
                                        {
                                            if(ObjectUtils.isObject(obj[prop][p]) && (obj[prop][p]["forEach"]==null)){
                                                extended[prop][p] = ObjectUtils.extend(false, copyAccessors_flag, true, obj[prop][p], extended[prop][p]);
...
*/
ObjectUtils.isObject = function (x) {
...
*/
ObjectUtils.getChainValue = function(host, chain){
    if(typeof chain === 'string')
        chain = ObjectUtils.parseObjectPathExpression(chain);
...
*/
ObjectUtils.setChainValue = function (host, chain, value, create=false) {
    if(typeof chain === 'string')
        chain = ObjectUtils.parseObjectPathExpression(chain);
...
*/
ObjectUtils.opath = {};
ObjectUtils.parseObjectPathExpression = function (str) {
...
*/
ObjectUtils.deepEqual = function(x, y, matchAllKeys = false, v) {
    ...
    return x && y && tx === 'object' && tx === ty && (!matchAllKeys || (ok(x).length==ok(y).length)) ? (
        ok(x).every((key) => {
            ...
            if(v.indexOf(x[key]) == -1){
                v.push(x[key]);
                r =  ObjectUtils.deepEqual(x[key], y[key], matchAllKeys, v);
            }
            return r;
        })
    ) : (x === y);
...
ObjectUtils.findMember = function (o, member, excludeProps=[], value=null, autoOptimize=true,/*internal*/ pn = [], visited=[]) {
    let ap = [];
    if (o.forEach) {
        let len = o.length;
        for (let i = 0; i < len; i++) {
            if (o[i] && (o[i].forEach || ObjectUtils.isObject(o[i]))) {
                let pni = pn.slice(0);
                pni.push(`${i}`);
                let d = ObjectUtils.findMember(o[i], member, excludeProps, value, autoOptimize, pni, visited);
                if (d.length > 0) {
                    ap.splicea(ap.length, 0, d);
                }
            }  
        }
    } else if (ObjectUtils.isObject(o)) { 
        for (let prop in o) { 
            if (excludeProps.indexOf(prop) < 0) { 
                ...
                if (prop == member && (value==null || (value!=null && o[prop]==value))) {
                    ap.push(pnp);
                } else if (o[prop] && (o[prop].forEach || ObjectUtils.isObject(o[prop]))) {
                    let ind = ArrayUtils.indexOfObject(visited, "instance", o[prop]);
                    if (ind < 0) { 
                        visited.push({"instance": o[prop], "path": pnp});
                        let d = ObjectUtils.findMember(o[prop], member, excludeProps, value, autoOptimize, pnp, visited);
                        if (d.length > 0) {
                            ap.splicea(ap.length, 0, d);
                        }
                    } else {
