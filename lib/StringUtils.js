...
*/
function getBindingExp(str){
    var exp = null;
    if (str && isString(str) && str.length>2 && str[0] == '{' && str[str.length - 1] == '}') {
        exp = {expression:null, nullable:false};
...
/**
 * Convert a string in which words are separated with the minus character to camelCase
 ...
 */
function sortBestMatch(ac, toMatch, matchType, key, maxCount = null){
    var ordered = [];
    let len = ac.length;
    ...
    {
        if(key)
            against = getChainValue(ac[i], key);
        else
            against = ac[i];
