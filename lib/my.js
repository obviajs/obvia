function testvalid(type,value)
{
	var validator = 
	{
		...
		url:{exp:"^(?!mailto:)(?:(?:http|https|ftp)://)(?:\\S+(?::\\S*)?@)?(?:(?:(?:[1-9]\\d?|1\\d\\d|2[01]\\d|22[0-3])(?:\\.(?:1?\\d{1,2}|2[0-4]\\d|25[0-5])){2}(?:\\.(?:[0-9]\\d?|1\\d\\d|2[0-4]\\d|25[0-4]))|(?:(?:[a-z\\u00a1-\\uffff0-9]+-?)*[a-z\\u00a1-\\uffff0-9]+)(?:\\.(?:[a-z\\u00a1-\\uffff0-9]+-?)*[a-z\\u00a1-\\uffff0-9]+)*(?:\\.(?:[a-z\\u00a1-\\uffff]{2,})))|localhost)(?::\\d{2,5})?(?:(/|\\?|#)[^\\s]*)?$"},
		urlip:{exp:"/^https?\:\/\/[^\/\s]+(\/.*)?$/"},
		uint:{exp:'^[0-9]+$'},
		sint:{exp:'^[-]?[0-9]+[\.]?[0-9]+$'}
	}; 
...
jQuery.fn.attributes = function(all=false) {
	...
	let obj = {}, len = this[0].attributes.length;
	for(let i=0;i<len;i++)
	{
...
function get(url, mimeType="application/json") {   
	return new Promise((resolve, reject) => {
		...
		xhr.onreadystatechange = function () {
			if (xhr.readyState == 4) {
				resolve({"status":this.status, "response": xhr.response || xhr.responseText});
			}
		};
