...
QueryBuilder.prototype.bindEvents = function() {
    ...
    this.$el.on('change.queryBuilder', Selectors.rule_operator, function() {
        var $rule = $(this).closest(Selectors.rule_container);
        var $model = self.getModel($rule);
        $model.operator = self.getOperatorByType($(this).val());
        ...
        if (allowedOperators.indexOf($model.operator.type) != -1) {
            $('#_field_compare_' + $model.id + 0).css('display', 'block');
            $('#field_compare_' + $model.id + 0).prop('checked', false);
            $('#switch_input_' + $model.id + 0).css('display', 'none');
            $('#input_' + $model.id + 0).css('display', 'block');
        }
        else {
            $('#field_compare_' + $model.id + 0).prop('checked', false);
            $('#_field_compare_' + $model.id + 0).css('display', 'none');
            $('#switch_input_' + $model.id + 0).css('display', 'none');
            $('#input_' + $model.id + 0).css('display', 'block');
        }
...
QueryBuilder.prototype.createRuleFilters = function(rule) {
    ...
    /**
     * Added Select2 For Select input types
     */
    rule.$el.find(QueryBuilder.selectors.filter_container).children().select2({ dropdownAutoWidth: true });
...
QueryBuilder.prototype.createRuleOperators = function(rule) {
     ...
     * Added Select2 For Select input types
     */
    $operatorContainer.children().select2({ dropdownAutoWidth: true });
...
QueryBuilder.prototype.createRuleInput = function(rule) {
    ...
    for (var i = 0; i < rule.operator.nb_inputs; i++) {
        ...
        if (i > 0) $valueContainer.append('<span style="float: left"> ' + this.settings.inputs_separator + ' </span>');
         ...
         * Added Select2 For Select input types
         */
        if ($("#input_" + rule.id + i).is('select'))
            $("#input_" + rule.id + i).select2({ dropdownAutoWidth: true });
    }
...
QueryBuilder.prototype.updateRuleFilter = function(rule, previousFilter) {
    ...
    rule.$el.find(QueryBuilder.selectors.rule_filter).select2('val', rule.filter ? rule.filter.id : '-1');
...
QueryBuilder.prototype.updateRuleOperator = function(rule, previousOperator) {
    ...
    if (rule.operator) {
        rule.$el.find(QueryBuilder.selectors.rule_operator).select2('val', rule.operator.type);
    }
        ...
        group.each(function(rule) {
            ...
            for (var i = 0; i < rule.operator.nb_inputs; i++) {
                if (rule.operator.nb_inputs > 1) {
                    if ($('#field_compare_' + rule.id + i).is(':checked')) {
                        ruleData.value[i] = $('#switch_input_' + rule.id + i).val();
                        ruleData.self += (i + 1);
                    }
                }
                else {
                    if ($('#field_compare_' + rule.id + 0).is(':checked')) {
                        ruleData.value = $('#switch_input_' + rule.id + 0).val();
                        ruleData.self = 0;
                    }
                }
            }
    ...
    (function add(data, group) {
        ...
        data.rules.forEach(function(item) {
            ...
            if (item.self !== "") {
                ...
                switch (String(item.self)) {

                    case '0':
                        $('#field_compare_' + item.input_id + 0).prop('checked', true);
                        $('#input_' + item.input_id + 0).css('display', 'none');
                        $('#input_' + item.input_id + 0).val('');
                        $('#switch_input_' + item.input_id + 0).css('display', 'block');
                        $('#switch_input_' + item.input_id + 0).val(item.value);
                        break;
                    ...
                    case '1':
                        ...
                        $('#input_' + item.input_id + 0).val('');
                        $('#switch_input_' + item.input_id + 0).css('display', 'block');
                        $('#switch_input_' + item.input_id + 0).val(item.value[0]);
                        break;

                    case '2':
                        $('#field_compare_' + item.input_id + 1).prop('checked', true);
                        $('#input_' + item.input_id + 1).css('display', 'none');
                        $('#input_' + item.input_id + 1).val('');
                        $('#switch_input_' + item.input_id + 1).css('display', 'block');
                        $('#switch_input_' + item.input_id + 1).val(item.value[1])
                        break;
...
QueryBuilder.prototype.validateValue = function(rule, value) {
    ...
    if (rule.operator.nb_inputs == 2) {
        if ($('#field_compare_' + rule.id + 0).is(':checked') && $('#field_compare_' + rule.id + 1).is(':checked')) {
            result = true;
        }
        if ($('#field_compare_' + rule.id + 0).is(':checked') && value[1] != "") {
            result = true;
        }
        if ($('#field_compare_' + rule.id + 1).is(':checked') && value[0] != "") {
            result = true;
        }
    }

    if (rule.operator.nb_inputs == 1 && $('#field_compare_' + rule.id + 0).is(':checked')) {
        result = true;
    }
...
QueryBuilder.prototype.getRuleInput = function(rule, value_id) {
        ...
        switch (filter.input) {
            ...
            case 'checkbox':
                Utils.iterateOptions(filter.values, function(key, val) {
                    h += '<label' + c + '><input type="' + filter.input + '" id="input_' + rule.id + value_id + '" name="' + name + '" value="' + key + '"> ' + val + '</label> ';
                });
                break;

            case 'select':
                h += '<select class="form-control form-control-custom" id="input_' + rule.id + value_id + '" name="' + name + '"' + (filter.multiple ? ' multiple' : '') + '>';
                if (filter.placeholder) {
            ...
            case 'textarea':
                h += '<textarea class="form-control" id="input_' + rule.id + value_id + '" name="' + name + '"';    
            ...
            case 'number':
                h += '<input class="form-control" type="number" style="float: left" id="input_' + rule.id + value_id + '" name="' + name + '"';    
            ...
            default:
                h += '<input class="form-control" type="text" style="float: left" id="input_' + rule.id + value_id + '" name="' + name + '"';
                if (filter.placeholder) h += ' placeholder="' + filter.placeholder + '"';
    ...
    h += '<span><select id="switch_input_' + rule.id + value_id + '" style="display: none;float: left;" onclick="myEventHandler(event);" class="form-control form-control-custom">';

    for (var i = 0; i < this.settings.filters.length; i++) {
        if (rule.__.filter.input == this.settings.filters[i].input && rule.__.filter.type == this.settings.filters[i].type && rule.__.filter.id != this.settings.filters[i].id) {
            h += '<option value="' + this.settings.filters[i].id + '">' + this.settings.filters[i].label + '</option>';
            exists++;
        }
    }
    h += '</select></span>';

    if (exists != 0 && filter.input != "radio" && filter.input != "select" && filter.input != "checkbox") {
        h += '<span><label id="_field_compare_' + rule.id + value_id + '" style="float: left;padding-left: 10px;padding-top: 5px;" onclick="myEventHandler(event);switchInput(\'' + rule.id + '\', ' + value_id + ');"><input type="checkbox" id="field_compare_' + rule.id + value_id + '" /><b>Field Compare</b></label></span>';
    }
...
/**
 * @class HumanReadableCondition
 * @memberof module:plugins
 * @description Produces a Human Readable Sentence From Condition.
 */
QueryBuilder.define('hrc', function () { 
    var self = this;
    var Selectors = QueryBuilder.selectors;
    
    function generateHRCString(cond, level) {
        try {
            cond = JSON.parse(cond);
            var str_cond = "";

            if (cond.rules.length > 0) {
                if (cond.rules[0].hasOwnProperty('condition') && cond.rules[0].condition != null) {
                    str_cond = "<br>" + String(".").repeat(level) + str_cond;
                    str_cond = str_cond + "(" + generateHRCString(JSON.stringify(cond.rules[0]), level + 1) + "<br>" + String(".").repeat(level) + ")";
                } else {
                    var field = cond.rules[0].field;
                    str_cond = "<br>" + String(".").repeat(level) + str_cond;
                    str_cond = str_cond + "(" + self.settings.hrcOperators[cond.rules[0].operator](field, cond.rules[0].type, cond.rules[0].value, cond.rules[0].self) + ")";

                    cond.rules.splice(0, 1);
                    if (cond.rules.length > 0) {
                        str_cond += " " + self.settings.hrcLogicalOperators[cond.condition] + " " + generateHRCString(JSON.stringify(cond), level) + " ";
                    }
                }
            }
    ...
    function appendString($el, rules) {
        $el
            .html(
            "<code>" +
            generateHRCString(JSON.stringify(rules)) +
            "</code>"
            );
    }

    window.toggleHRC = function (id) {
        if (document.getElementById(id + "-hrc-container").style.display == 'none') {
            $("#" + id + "-hrc-container").css('display', 'block');
            $("#" + id + "_group_0").css({ 'width': '75%', 'float': 'left' });
        } else {
            $("#" + id + "-hrc-container").css('display', 'none');
            $("#" + id + "_group_0").css({ 'width': '100%', 'float': 'left' }); 
        }      
    }
    ...
    self.on('afterInit', function () {
        ...
        self.on('getGroupTemplate.filter', function (h, level) { 
            
            self.on('afterSetRules afterUpdateRuleFilter', function () {
        
                var $el =
                    $("#" + this.id + "_group_0")
                        .parent();
                
                $el.find("#" + this.id + "-hrc-container").remove();
                $el.append("<dl id='" + this.id + "-hrc-container' class='rules-group-container' style='display:none; width:25%; float:right; min-height:100px;'></dl>")

                $el.find('.hrc-info-btn').remove();
                $el
                    .find('.btn-group')
                    .first()
                    .append("<a href='javascript:void(0)' onclick='toggleHRC(\"" + this.id + "\")' class='hrc-info-btn btn btn-xs btn-warning'><i class='fa fa-info-circle' aria-hidden='true'></i> Friendly Condition</a>");

                self.on('afterUpdateRuleValue afterUpdateRuleOperator afterUpdateGroupCondition afterInvert afterMove', function () {
                    appendString($("#" + this.id + "-hrc-container"), self.getRules());
                });
...
QueryBuilder.defaults({
    hrcOperators: {
        'equal': function (field, type, value, selfcompare) { 
            if (selfcompare === 0)
                selfcompare = true;
            else
                selfcompare = false;
            ...
            if (selfcompare) {
                return field + " eshte sa " + value;
            }
            return field + " eshte " + (["double", "integer"].includes(type) ? value : "'" + value + "'");
        },
        'not_equal': function (field, type, value, selfcompare) {
            if (selfcompare === 0)
                selfcompare = true;
            else
                selfcompare = false;
            ...
            if (selfcompare) {
                return field + " nuk eshte sa " + value;
            }
            return field + " nuk eshte " + (["double", "integer"].includes(type) ? value : "'" + value + "'");
        },
        'in': function (field, type, value, selfcompare) {
            return field + "ne (" + ((["double", "integer"].includes(type) ? value.join(',') : "'" + value.join('","') + "'")) + ")";
        },
        'not_in': function (field, type, value, selfcompare) {
            return field + "jo ne (" + ((["double", "integer"].includes(type) ? value.join(',') : "'" + value.join('","') + "'")) + ")";
        },
        'less': function (field, type, value, selfcompare) {
            if (selfcompare === 0)
                $selfcompare = true;
            else
                selfcompare = false;
            
            return field + " me e vogel se " + (["double", "integer"].includes(type) || selfcompare ? value : "'" +  value + "'");
        },
        'less_or_equal': function (field, type, value, selfcompare) {
            if (selfcompare === 0)
                $selfcompare = true;
            else
                selfcompare = false;

            return field + " me e vogel ose e barabarte me " + (["double", "integer"].includes(type) || selfcompare ? value : "'" + value + "'");
        },
        'greater': function (field, type, value, selfcompare) {
            if (selfcompare === 0)
                $selfcompare = true;
            else
                selfcompare = false;

            return field + " me e madhe se " + (["double", "integer"].includes(type) || selfcompare ? value : "'" + value + "'");
        },
        'greater_or_equal': function (field, type, value, selfcompare) {
            if (selfcompare === 0)
                $selfcompare = true;
            else
                selfcompare = false;

            return field + " me e madhe ose e barabarte me " + (["double", "integer"].includes(type) || selfcompare ? value : "'" + value + "'");
        },
        'between': function (field, type, value, selfcompare) {
            var selfcompare1, selfcompare2;

            if (selfcompare == "1") {
                selfcompare1 = true;
                selfcompare2 = false;
            }
            else if (selfcompare == "2") {
                selfcompare1 = false;
                selfcompare2 = true;
            }
            else if (selfcompare == "12") {
                selfcompare1 = true;
                selfcompare2 = true;
            }
            else {
            ...
            return field + " midis " + (["double", "integer"].includes(type) || selfcompare1 ? value[0] : "'" + value[0] + "'") + " dhe " + (["double", "integer"].includes(type) || selfcompare2 ? value[1] : "'" + value[1] + "'");
        },
        'not_between': function (field, type, value, selfcompare) {
            var selfcompare1, selfcompare2;
            ...
            return field + " jo midis " + (["double", "integer"].includes(type) || selfcompare1 ? value[0] : "'" + value[0] + "'") + " dhe " + (["double", "integer"].includes(type) || selfcompare2 ? value[1] : "'" + value[1] + "'");
        },
        'begins_with': function (field, type, value, selfcompare) {
            return field + " fillon me '" + value + "'";
        },
        'not_begins_with': function (field, type, value, selfcompare) {
            return field + " nuk fillon me '" + value + "'";
        },
        'contains': function (field, type, value, selfcompare) {
            return field + " permban '" + value + "'";
        },
        'not_contains': function (field, type, value, selfcompare) {
            return field + " nuk permban '" + value + "'";
        },
        'ends_with': function (field, type, value, selfcompare) {
            return field + " mbaron me '" + value + "'";
        },
        'not_ends_with': function (field, type, value, selfcompare) {
            return field + " nuk mbaron me '" + value + "'";
        },
        'is_empty': function (field, type, value, selfcompare) {
            return field + " eshte bosh";
        },
        'not_not_empty': function (field, type, value, selfcompare) {
            return field + " nuk eshte bosh";
        },
        'is_null': function (field, type, value, selfcompare) {
            return field + " eshte nul";
        },
        'is_not_null': function (field, type, value, selfcompare) {
            return field + " nuk eshte nul";
        },
    },
                    ...
                    if (ope.nb_inputs !== 0) {
                        ...
                        rule.value.forEach(function(v, i) {
                            ...
                            if (stmt) {
                                ...
                                if ($('#field_compare_' + rule.input_id + i).is(':checked')) {
                                    v = v.slice(1, -1);
                                }
...
function switchInput(rule, value) {

    if ($('#field_compare_' + rule + value).is(':checked')) {
        $('#input_' + rule + value).css('display', 'none');
        $('#switch_input_' + rule + value).css('display', 'block');
    }
    else {
        $('#switch_input_' + rule + value).css('display', 'none');
        $('#input_' + rule + value).css('display', 'block');
    }
}