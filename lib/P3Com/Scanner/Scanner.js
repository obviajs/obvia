...
var Scanner = function(p3com, divToShow) {
    ...
    _P3Com.on(P3ComEventType.COMMAND_COMPLETED, function(event, responseObj, command) {
        switch (command.commandType.command) {
            case "LoadScanners":
                ...
                localStorage.setItem("obvia.P3Com.Scanner.Scanners", JSON.stringify(_scanners));
    ...
    this.Scan = function(scanItem) {
        ...
        if (scanItem.deviceId) {
            if (this.Scanners) {
                var match = ArrayUtils.getMatching(this.Scanners, "deviceId", scanItem.deviceId, true);

                if (match.objects.length > 0) {
                    match = match.objects[0];

                    scanItem = ObjectUtils.extend(false, false, [], [], [], match, scanItem);
    ...
    Object.defineProperty(this, 'DeviceId', {
        get: function() {
            if (!_deviceId) {
                if (localStorage.getItem("obvia.P3Com.Scanner.DeviceId")) {
                    _deviceId = localStorage.getItem("obvia.P3Com.Scanner.DeviceId");
                }
            }
            return _deviceId;
        },
        set: function(dId) {
            _deviceId = dId;
            localStorage.setItem("obvia.P3Com.Scanner.DeviceId", _deviceId);
        }
    });
    ...
    Object.defineProperty(this, 'Scanners', {
        get: function() {
            if (!_scanners) {
                if (localStorage.getItem("obvia.P3Com.Scanner.Scanners")) {
                    _scanners = JSON.parse(localStorage.getItem("obvia.P3Com.Scanner.Scanners"));
                }
            }
            return _scanners;
        }
