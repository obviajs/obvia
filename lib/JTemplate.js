...
var JTemplate = function (_tpl, _context) {
    ...
    this.parse = function () {
        ...
        if (StringUtils.isString(_tpl)) {
            ...
            _tempObj = ObjectUtils.extend(true, false, _tpl);
    ...
    this.now = function (p, v) {
        v = v ? v : [];
        let snapShot = p || (_tempObj.forEach ? ArrayUtils.acExtend(true, _tempObj) : extend(true, false, _tempObj));
        v.push(snapShot);
        for (let key in _hash) {
            if (Array.isArray(snapShot)) {
                for (let i = 0; i < snapShot.length; i++) {
                    ...
                    if (snapShot[i] == key)
                        snapShot.splice(i, 1);
                    else if (ObjectUtils.isObject(snapShot[i]) || Array.isArray(snapShot[i])) {
                ...
                for (let prop in snapShot) {
                    ...
                    else if (ObjectUtils.isObject(snapShot[prop]) || Array.isArray(snapShot[prop])) {
                        let ind = v.indexOf(snapShot[prop]);
                        if (ind == -1 || ind >= len - 1)
    ...
    let _explore = function (obj, chain) {
        for (let prop in obj) {
            ...
            } else if (ObjectUtils.isObject(obj[prop]) || Array.isArray(obj[prop])) {
                let chainBranch = chain.slice(0);
                chainBranch.push(prop);
    ...
    let _processBindingsLiteral = function (tplObj) {
        for (let prop in tplObj) {
            if (StringUtils.isString(tplObj[prop]) && _regex.test(tplObj[prop])) {
                _regex.lastIndex = 0;
                ...
                tplObj[prop] = key;
            } else if (ObjectUtils.isObject(tplObj[prop]) || Array.isArray(tplObj[prop])) {
