...
*/
function objectHierarchyGetMatchingMember(object, key, matchingValue, childrenpropertypath='children', minDepth=0, maxDepth=1000, chain=null, cDepth=0)
{
    if(!chain)
        chain = [];
    if (cDepth == null) { 
    ...
    let ret = { "chain": chain, "match": null };
    ...
    for (var mid in toExplore)
    {
        ...
        if (cDepth > minDepth && cDepth < maxDepth && getChainValue(member, key) == matchingValue) {
            ret = { "chain": chain, "match": member };
            break;
        }else if(member && cDepth < maxDepth)
        {
            ++cDepth;
            let tmp = objectHierarchyGetMatchingMember(member, key, matchingValue, childrenpropertypath, minDepth, maxDepth, chain, cDepth);
            if (tmp != null) { 
                ret = { "chain": chain, "match": tmp.match };
                break;
            }
        } 
    }
...
function objectHierarchyGetMatching(object, key, matchingValue, childrenproperty='children', minDepth=0, maxDepth=1000, chain=null, cDepth=0)
{
    ...
    }else if(object[childrenproperty] && cDepth < maxDepth)
    {
        ...
        if(tmp != null) return {"chain":chain, "match": tmp.match};
    }
    return ret;
}
...
function acSort(ac, key, direction = 1){
    ac.sort(function(a, b){
        let c = -1;
        if((key && getChainValue(a, key) >= getChainValue(b, key) || a>=b) && direction==1){
            c = 1;
        }
        return c; 
    });
...
function differenceOnKeyMatch (a1, a2, key, bidirectional = false, retIndices=false, startIndex=0) {
    ...
    if (!a2 || a2.length==0){
        ...
        if(a1){
            let len = a1.length;
            for (let i = startIndex; i < len; i++) {
                if ((key==null && a2.indexOf(a1[i])===-1) || (indexOfObject(a2, key, getChainValue(a1[i], key)) === -1)) {
                    result.push(a1[i]);
                    indices.push(i);
                }
            }
...
Object.defineProperty(Array.prototype, "togglea", {enumerable: false});
/**
