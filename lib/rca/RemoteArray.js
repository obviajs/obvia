...
var RemoteArray = function(p)
{
	...
	var props = extend(_defaultParams, p);

	this.$el = $(this);
    var _recordsPerPage = props.recordsPerPage;
	var _currentPageIndex = -1;
	var _totalRecords = 0;		 
	var _sortData = props.sortData;	 
	var _filterData = props.filterData;
	var _lastop = 0;
	var _lastopRec = 0;
	...
	var _source = [];
	var _busy = false;
	var _threshold = props.threshold;
	...
	var _autoRetry = props.autoRetry;
	var _autoReset = props.autoReset;
	var _post = props.post;
			...
			if(data.collection != null)
			{
				...
				if(data.collection["length"] !=undefined && data.collection.length > 0)
				{
					if(_cumulative)
					{
						...
						_source.splicea(_source.length, 0, data.collection);
					}
					else
					{
						_source.splicea(0, _source.length, data.collection);
					}
					_loadedPagesCount = 1;
					if(!isPreload){
				...
				if(data.collection.length/_recordsPerPage<expectedPages){
					_totalRecords = _source.length;
				}
			}
		}else
	...
	{		
		if(!_busy){
			...
			// }else{
				...
				var _postExt = extend({isSynced: _isSynced, startPage: _startPage, endPage: _endPage, recordsPerPage: _recordsPerPage, sortData: _sortData, filterData: _filterData}, _post);
			
				simpleAjax(_url, _postExt, 
					function (result) { _fnUpdateSuccess.call(thisContext, result, isPreload, _endPage-_startPage);}, 
					function (jqXHR, textStatus, errorThrown) { _fnUpdateFailed.call(thisContext, jqXHR, textStatus, errorThrown, isPreload);});		
			// }
		}else
		{
			//TODO: hidh eventin
