...
{strObjPath}
    ...
    let _generate = function (oas)
    {
        ...
        let apiTitle = StringUtils.convertToCamelCase(oas.info.title + _title);
                    ...
                    for (let r in oas.paths[path][method].responses)
                    {
                        for (let ct in oas.paths[path][method].responses[r].content)
                        {
                            ...
                            if (oas.paths[path][method].responses[r].content[ct].schema.type)
                            {
                                ...
                                typeInfo.title = typeInfo.title || StringUtils.convertToCamelCase(method + "ResponseBody" + "_" + path.split("/").last());
                                typeInfo.type = typeInfo.type || "object";
...
{properties}
        for(let prop in _props){
            if(!this.hasOwnProperty(prop)){
                this[prop] = new {fqType}(_props[prop]);
            }
        }
    };`;
    ...
    let _parseType = function (oas, typeInfo, propName, /*internal*/ visited = {})
    {
                    ...
                    if (ObjectUtils.isObject(typeInfo.additionalProperties) && !ObjectUtils.isEmpty(typeInfo.additionalProperties))
                    {
                        for (let prop in typeInfo.additionalProperties)
                        {
                            ...
                            if (subTypes[propName] == null)
                            {
                                ...
                                let fqType = "";
                                if (_primitives[childType])
                                {
                                    fqType = childType;
                                } else
                                {
                                    fqType = "{apiTitle}." + childType;
                                }
        ...
        } else if (typeInfo.type == "array")
        {
            isArray = true;
            let allowedTypes = [];
            //Swashbuckle doesn`t generate oneOf when there is only on type allowed...
            if (typeInfo.items.$ref)
            {
                typeInfo.items.oneOf = [{ "$ref": typeInfo.items.$ref }];
            }
            if (typeInfo.items.oneOf)
            {
                for (let i = 0; i < typeInfo.items.oneOf.length; i++)
                {
                    for (let prop in typeInfo.items.oneOf[i])
                    {
                        ...
                        {
                            let typePath = typeInfo.items.oneOf[i].$ref;
                            let typeChain = typePath.split('/');
                            let elementJsType = typeChain.last();
                            if (visited[elementJsType] == null)
                            {
                            ...
                            allowedTypes.push(elementJsType);
                        } else
                        {
            ...
            {
                if (_oasjsMap[typeInfo.items.type])
                {
                    allowedTypes.push(_oasjsMap[typeInfo.items.type]);
                } else
                {
                    ...
                    {
                        myTypeNames.splicea(myTypeNames.length, 0, r.typeNames);
                        allowedTypes.push(r.typeName);
                    }
                }
            }
            arrTypeName = StringUtils.convertToCamelCase("array-" + allowedTypes.join("-"));
            if (!propName)
                propName = arrTypeName;
        ...
        } else if (typeInfo.$ref)
        {
            ...
            if (typeNames.indexOf(propName) < 0 && visited[jsType] == null)
            {
                visited[jsType] = 1;
                let r = _parseType(oas, typeInfo, jsType, visited);
                visited[jsType] = r;
                if (r.typeNames.length > 0)
                {
                    myTypeNames.splicea(myTypeNames.length, 0, r.typeNames);
                ...
                //+ _typeTemplate.formatUnicorn({"jsDoc":r.jsDoc, "typeName": propName, "typeProperties":r.properties});                               
            } else
                myTypeNames = subTypes[propName] ? subTypes[propName] : [];
